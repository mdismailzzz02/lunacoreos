...
export function useJournal() {
    ...
    const load = useCallback(async () => {
        try {
            ...
            // If Supabase returns 403, it's likely an auth / RLS issue — prompt user to sign in
            if (e && (e.status === 403 || (e.message && e.message.toLowerCase && e.message.toLowerCase().includes('forbidden')))) {
                addToast('Access denied — please sign in to view your journal', 'error');
            } else {
    ...
    const updateSilently = async (params) => {
        try {
            await api.updateEntry(params);
            setEntries(prev => prev.map(e => e.entry_id === params.entry_id ? { ...e, ...params } : e));
        } catch (e) {
            console.error('Silent entry save failed:', e);
        }
    };
    ...
    return { entries, loading, saving, create, update, updateSilently, remove, refresh: load };
}
