...
import { Heart, Library, Globe, Flame, Plus, MonitorPlay, Inbox, X, Check, Search, Radio } from 'lucide-react';
            ...
            <div className="yt-thumb-wrap" style={{ position: 'relative' }}>
                ...
                <div style={{ position: 'absolute', top: '6px', right: '6px', display: 'flex', gap: '6px', alignItems: 'center' }}>
                    ...
                    {onSaveChannel && (
                        ...
                        >
                            {isChannelSaved ? <MonitorPlay size={14} color="white" /> : <Plus size={14} color="white" />}
                        </button>
                    )}
            ...
            <div className="yt-pending-actions" style={{ flexDirection: 'column', gap: '8px', alignItems: 'flex-end' }} onClick={e => e.stopPropagation()}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                    ...
                    <button className="yt-approve-btn" onClick={handleApprove} disabled={saving} title="Add to Library">
                        {saving ? '…' : <Check size={14} color="currentColor" />}
                    </button>
                </div>
                {shouldDelegate && (
                    <input
...
export default function Videos() {
    ...
    const fetchVideos = useCallback(async (chans, sel, ignored, savedSet) => {
        ...
        try {
            ...
            const resultsRaw = await Promise.allSettled(targets.map(c => getChannelVideos(c.uploadsId, sel ? 20 : 8)));
            const results = resultsRaw.filter(r => r.status === 'fulfilled').map(r => r.value);
            
            if (results.length === 0 && targets.length > 0) {
                throw new Error('All channels failed to load');
            }
        ...
        <div className="videos-layout apple-page-loaded">
            ...
            <div className="vault-mobile-nav mobile-only" style={{ marginBottom: '1.25rem' }}>
                <div className="vault-segments">
                    <button className={activeTab === 'feed' ? 'active' : ''} onClick={() => setActiveTab('feed')}>
                        <Flame size={16} style={{ marginRight: '6px' }} /> Feed
                    </button>
                    ...
                    <button className={activeTab === 'channels' ? 'active' : ''} onClick={() => setActiveTab('channels')}>
                        <Radio size={16} style={{ marginRight: '6px' }} /> Channels
                    </button>
                </div>
            </div>
            ...
            <aside className="videos-sidebar desktop-only">
                <div className="videos-sidebar-header">
                    <h2 className="section-title" style={{ marginBottom: 0, display: 'flex', alignItems: 'center', gap: '8px' }}><MonitorPlay size={18} /> Channels</h2>
                </div>
                ...
                {channels.length > 0 && (
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '4px', margin: '0.75rem 0' }}>
                        ...
                        >
                            <Globe size={18} style={{ marginRight: '8px' }} color="var(--text-muted)" /> All Feed
                        </button>
                        <button 
                            className={`yt-all-btn ${activeTab === 'search' ? 'active' : ''}`} 
                            onClick={() => { setActiveTab('search'); setSelected(null); }}
                        >
                            <Search size={18} style={{ marginRight: '8px' }} color="var(--text-muted)" /> Search YouTube
                        </button>
                        <button 
                ...
                {activeTab === 'feed' && channels.length > 0 && (
                    <div className="mobile-channel-strip mobile-only fade-in">
                        <div className={`strip-item ${!selected ? 'active' : ''}`} onClick={() => setSelected(null)}>
                            <div className="strip-avatar-all" style={{ display: 'flex', alignItems: 'center', justifyContent: 'center' }}><Globe size={20} /></div>
                            <span>All</span>
                        </div>
                        {channels.map(ch => (
                        ...
                        {(activeTab === 'feed' && !selected) && filteredLibrary.length > 0 && (
                            <div className={pending.length > 0 ? 'yt-approved-section' : ''} style={{ marginTop: '1.5rem' }}>
                                <div className="yt-approved-header" style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                                    <Library size={18} /> Saved from Library
                                </div>
                        ...
                        {activeTab === 'liked' && (
                            <div className="yt-approved-section">
                                ...
                                {likedVideoIds.size === 0 ? (
                                    <div className="empty-state">
                                        <span className="empty-emoji">🤍</span>
                                        <p>No liked videos yet. Tap <Heart size={14} fill="#ef4444" color="#ef4444" style={{ display: 'inline', verticalAlign: 'middle', margin: '0 2px' }} /> on any video to save it here.</p>
                                    </div>
                                ) : (
                                    <div className="yt-video-grid">
